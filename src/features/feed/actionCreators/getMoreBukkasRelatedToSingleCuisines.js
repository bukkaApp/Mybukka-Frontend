...
const getMoreBukkasRelatedToSingleCuisinesAction = (type, data, currentPage) => ({
...
const getMoreBukkasRelatedToSingleCuisines = (
  ...
  try {
    ...
    dispatch(getMoreBukkasRelatedToSingleCuisinesAction('SUCCESS', request.data, page));
  } catch (error) {
    dispatch(loading(FETCH_MORE_BUKKAS_RELATED_TO_SINGLE_CUISINE, false));
    dispatch(getMoreBukkasRelatedToSingleCuisinesAction('ERROR', error.response.data));
  }
};
